...
export default function DeviceOnboardingPage() {
  ...
  // 1. Verify invite token on mount
  useEffect(() => {
    if (!token) {
      setStatus('error');
  ...
  // 2. Handle device authorization via secure token
  const handleAuthorize = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Store the credential in a secure cookie so the proxy accepts this device
      const isSecure = window.location.protocol === 'https:';
      document.cookie = `bmtech_hardware_verified=${result.credentialId}; path=/; max-age=${60 * 60 * 24 * 365}; SameSite=Lax${isSecure ? '; Secure' : ''}`;
    ...
    <div className="min-h-screen bg-[#0A0B10] text-white flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-md z-10">
        ...
        <div className="bg-surface/50 backdrop-blur-xl border border-white/5 rounded-[32px] p-8 shadow-2xl space-y-8">
              ...
              <div className="space-y-4">
                <div className="p-4 rounded-2xl bg-emerald-500/5 border border-emerald-500/10 flex items-start gap-3">
                  ...
                  <p className="text-xs text-text-secondary leading-relaxed">
                    This invitation is valid. Name your device and click authorize to add it
                    to the trusted hardware list.
                  </p>
                </div>

                <InputField
                  label="Device Friendly Name"
                  placeholder="e.g., Office Desktop, My iPhone"
                  value={deviceName}
          ...
          {status === 'registering' && (
            <div className="py-10 text-center space-y-6">
              ...
              <div className="space-y-2">
                <h4 className="text-xl font-bold">Authorizing Device...</h4>
                <p className="text-text-secondary text-sm italic">
                  Generating secure device credentials...
                </p>
              </div>
            </div>
          )}
          ...
          {status === 'success' && (
            <div className="py-10 text-center space-y-6 animate-in zoom-in-95 duration-500">
              ...
              <div className="space-y-2">
                <h4 className="text-2xl font-bold">Device Authorized!</h4>
                <p className="text-text-secondary text-sm">
        ...
        <p className="mt-8 text-center text-[10px] text-text-secondary/50 uppercase tracking-[0.2em] font-medium">
          Zero-Trust Security &bull; Token Protected
        </p>
      </div>
    </div>
  );
