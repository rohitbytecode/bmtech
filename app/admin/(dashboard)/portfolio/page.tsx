...
const CATEGORIES = [
  ...
  { value: "Video Production & Multimedia Services", label: "Video Production & Multimedia Services" },
...
const projectColumns: Column<Project>[] = [
  ...
  { 
    header: "Live URL", 
    accessor: (project) => (
      <a href={formatUrl(project.link)} target="_blank" rel="noopener noreferrer" className="text-accent-blue hover:underline font-medium">
        View Site
      </a>
    )
  },
  ...
  return (
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-6 duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <Button onClick={() => { setEditingProject(null); setIsOpen(true); }} className="h-14 px-8 gap-2 group">
      ...
      <ModalForm
        isOpen={isOpen}
        onClose={() => { setIsOpen(false); setEditingProject(null); }}
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <InputField name="title" label="Project Title" placeholder="e.g., Agency Website" required defaultValue={editingProject?.title} />
          <SelectField 
            name="category" 
            label="Category" 
            options={CATEGORIES} 
          ...
          />
          <InputField name="link" label="Live URL" placeholder="e.g., https://bmtech.com" required defaultValue={editingProject?.link} />
          <div className="md:col-span-2">
            <InputField name="image" label="Image URL" placeholder="e.g., /assets/project.jpg" defaultValue={editingProject?.image} />
          </div>
        </div>
      </ModalForm>
    </div>
