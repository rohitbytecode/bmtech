...
export default function StrategiesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const parsedIndustries = formData.target_industries
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean);
    const parsedCountries = formData.target_countries
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean);
    const parsedCities = formData.target_cities
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean);
  ...
  const columns = [
    {
      header: 'Name',
      accessor: 'name' as keyof Strategy,
    ...
    },
    {
      header: 'Description',
      accessor: (s: Strategy) => (
        <span className="truncate max-w-[250px] inline-block text-text-secondary text-xs">
          {s.description || '-'}
        </span>
      ),
    },
    {
      ...
      accessor: (s: Strategy) => (
        <span className="text-xs truncate max-w-[150px] inline-block">
          {s.target_countries?.length > 0 ? s.target_countries.join(', ') : 'Global'}
        </span>
      ),
    },
    {
      ...
      accessor: (s: Strategy) => (
        <span className="text-xs truncate max-w-[150px] inline-block">
          {s.target_industries?.length > 0 ? s.target_industries.join(', ') : 'All'}
        </span>
      ),
    },
    {
      ...
      <ModalForm
        ...
        description={
          editingId
            ...
            : 'Define a new outbound marketing strategy.'
        }
        isOpen={isModalOpen}
