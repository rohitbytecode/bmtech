...
export async function POST(request: Request) {
  try {
    ...
    if (!user) {
      // Prevent user enumeration by gracefully falling back
      return NextResponse.json(
        {
    ...
    console.log(
      `[API] Login Options request for ${email}. Host: ${host}, RP_ID: ${overrideRpId}, Hint: ${rpIdHint}`,
    );

    const options = await webauthnUtils.getAuthenticationOptions(
      credentials || [],
      overrideRpId,
      rpIdHint,
    );
    ...
    console.log(
      `[WebAuthn] Challenge generated for ${email}. RP_ID: ${overrideRpId}`,
    );
