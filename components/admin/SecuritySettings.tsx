...
import { Shield, Monitor, Smartphone, Trash2, Plus, Loader2, HardDrive, ExternalLink, Key, AlertTriangle, Edit2, CheckCircle2 } from "lucide-react";
...
export function SecuritySettings() {
  ...
  const currentDeviceInList = authorizedDevices.find(d => d.credential_id === currentCredentialId);
  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-right-4 duration-500">
      <div className="space-y-1 border-l-4 border-accent-blue pl-6">
        ...
        <p className="text-text-secondary text-sm">Manage hardware-backed passkeys for secure admin access.</p>
      </div>
      ...
      {authorizedDevices.length === 0 && (
        <div className="p-6 bg-rose-500/10 border border-rose-500/20 rounded-3xl space-y-3 animate-pulse">
          ...
          <p className="text-sm text-text-secondary">
            Your account is currently vulnerable. Register your first device to enable strict protection.
          </p>
        </div>
      )}
      ...
      <div className="p-8 rounded-3xl bg-surface border border-border space-y-8 shadow-xl shadow-black/10">
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            ...
            <p className="text-text-secondary text-sm">One-time codes to access your account if you lose your hardware.</p>
          </div>
          <Button onClick={generateRecoveryCodes} disabled={isGeneratingCodes} variant="outline" className="gap-2">
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4 p-6 bg-background/50 rounded-2xl border border-border font-mono text-sm">
              {recoveryCodes.map((code, i) => (
                <div key={i} className="text-text-primary">{i + 1}. {code}</div>
              ))}
            </div>
            <Button onClick={downloadRecoveryCodes} className="bg-accent-blue w-full">Download Keys</Button>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
        ...
        <div className="p-8 rounded-3xl bg-surface border border-border flex flex-col items-center text-center space-y-6 shadow-xl shadow-black/10">
          <div className={cn("h-16 w-16 rounded-2xl flex items-center justify-center", currentDeviceInList ? "bg-emerald-500/10 text-emerald-400" : "bg-accent-blue/10 text-accent-blue")}>
          ...
          {currentDeviceInList ? (
            <Button onClick={() => openRenameModal({ id: currentDeviceInList.id, name: currentDeviceInList.device_name })} variant="outline" className="w-full gap-2 border-emerald-500/20 text-emerald-400">
      ...
      <div className="bg-background border border-border rounded-3xl overflow-hidden shadow-2xl shadow-black/30">
        <div className="p-6 border-b border-border flex items-center justify-between bg-surface/50">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-bold text-text-primary uppercase tracking-widest">Registered Hardware ({authorizedDevices.length})</span>
          </div>
        </div>
        ...
        <div className="divide-y divide-border">
          {authorizedDevices.map((device) => {
            ...
            return (
              <div key={device.id} className={cn("p-6 flex items-center justify-between group transition-colors", isCurrent ? "bg-accent-blue/5" : "hover:bg-surface/50")}>
                <div className="flex items-center gap-5">
                  <div className={cn("h-12 w-12 rounded-2xl border flex items-center justify-center shadow-inner", isCurrent ? "bg-accent-blue/10 border-accent-blue/20 text-accent-blue" : "bg-surface border-border text-text-secondary")}>
                    <Monitor size={22} />
                  </div>
                  <div>
                    <div className="flex items-center gap-3">
                      <h5 className="font-bold text-text-primary">{device.device_name}</h5>
                      {isCurrent && <span className="px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 text-[10px] font-bold uppercase tracking-wider border border-emerald-500/20">Current Session</span>}
                    </div>
                    <div className="flex items-center gap-2 mt-1">
                      <p className="text-[10px] font-mono text-text-secondary/70">ID: {device.credential_id.substring(0, 12)}...</p>
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => openRenameModal({ id: device.id, name: device.device_name })} className="p-3 text-text-secondary hover:text-accent-blue hover:bg-accent-blue/10 rounded-xl transition-all opacity-0 group-hover:opacity-100">
                    <Edit2 size={20} />
                  </button>
                  <button onClick={() => handleRemoveDevice(device.id, device.device_name)} className="p-3 text-rose-400 hover:bg-rose-500/10 rounded-xl transition-all opacity-0 group-hover:opacity-100">
                    <Trash2 size={20} />
                  </button>
                </div>
              </div>
      ...
      {/* Register Modal */}
      <ModalForm isOpen={isRegModalOpen} onClose={() => setIsRegModalOpen(false)} title="Register Passkey" submitLabel={isRegistering ? "Confirming..." : "Register Device"} onSubmit={handleRegisterLocal} disabled={isRegistering}>
        <InputField label="Friendly Name" placeholder="e.g., My Office iMac" value={regDeviceName} onChange={(e) => setRegDeviceName(e.target.value)} required />
      </ModalForm>
