...
import { SCORING_CONFIG_V1 } from './config';
import type {
...
// Helper: Find signal by category and key
function findSignal(
...
/**
 * 1. Calculate Web Opportunity Score (0-100)
 */
export function calculateWebOpportunity(
  ...
  if (unreachableSig) {
    let score: number = cfg.website_unreachable_base;
    contributors.push({
  ...
  if (httpErrorSig) {
    let score: number = cfg.website_http_error_base;
    contributors.push({
...
export function calculateSeoOpportunity(
  ...
  // Branch 1: If no website or unreachable website exists, on-page SEO is 0 (service needed is Web, not SEO)
...
/**
 * 3. Calculate Marketing Opportunity Score (0-100)
 */
...
/**
 * 4. Calculate Design Opportunity Score (0-100)
 */
export function calculateDesignOpportunity(
  ...
  // Branch 1: No functional website
  if (isNoWeb || isUnreachable) {
    contributors.push({
  ...
  // Viewport check (Major responsive failure)
  const missingViewport = findSignal(signals, 'design', 'mobile_viewport_missing');
  if (missingViewport) {
  ...
  // Structural markup (Outdated table/div layout without semantic tags)
  const minimalContent = findSignal(signals, 'design', 'minimal_content');
  if (minimalContent) {
  ...
  // OpenGraph Image (Social branding preview card)
...
/**
 * 5. Calculate Data Quality & Business Validity
 */
...
/**
 * 6. Synthesize Overall Opportunity Score (0-100)
 */
...
/**
 * 7. Derive Sales Priority with Business Validity Gating
 */
export function deriveSalesPriority(
  ...
  // Case 1: High opportunity candidate (>= 75)
  ...
  // Case 2: Moderate opportunity candidate (50 - 74)
  ...
  // Case 3: Low-Moderate opportunity candidate (25 - 49)
  ...
  // Case 4: Minimal detected opportunity (< 25)
  return {
    priority: 'low',
    gated: false,
...
/**
 * 8. Main Entrypoint: Calculate All Opportunity Scores & Explanations
